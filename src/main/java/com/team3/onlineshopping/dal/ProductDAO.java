...
public class ProductDAO extends DBContext implements IDAO<Product> {
    ...
    public List<Product> filterProductByProNameAndSize(String price, String[] size, int cateProId, int cateProDeId) {
        List<Product> list = new ArrayList<>();
        String sql = "select Distinct p.productid, p.productname, p.ProductImageDefault, p.ProductPrice, p.ProductCost,p.ProductDescription,p.ProductRating,p.ProductSold,p.ProductCreatedDate,p.ProductStatus,p.EmployeeId,p.CategoryProductDetailsId from product as p\n"
                + "join productsize as ps on p.productid = ps.productid\n"
                + "join categorysize as cs on ps.categorysizeid = cs.categorysizeid\n"
                + "join categoryProductDetails as cpd on p.CategoryProductDetailsId = cpd.CategoryProductDetailsId\n"
                + "WHERE cpd.CategoryProductId = ? AND p.ProductStatus LIKE 'on'\n";
        ...
        if (size.length != 0) {
            ...
            sql += " AND cs.categorysizename IN (" + getSize + ")";
        }
    ...
    public List<Product> filterProductByProNameAndSizePaging(String price, String[] size, int indexPage, int cateProId, int cateProDeId) {
        List<Product> list = new ArrayList<>();
        String sql = "select Distinct p.productid, p.productname, p.ProductImageDefault, p.ProductPrice, p.ProductCost,p.ProductDescription,p.ProductRating,p.ProductSold,p.ProductCreatedDate,p.ProductStatus,p.EmployeeId,p.CategoryProductDetailsId from product as p\n"
                + "join productsize as ps on p.productid = ps.productid\n"
                + "join categorysize as cs on ps.categorysizeid = cs.categorysizeid\n"
    ...
    public List<Product> filterSearchProductByProNameAndSize(String price, String[] size, String searched) {
        List<Product> list = new ArrayList<>();
        String sql = "select Distinct p.productid, p.productname, p.ProductImageDefault, p.ProductPrice, p.ProductCost,p.ProductDescription,p.ProductRating,p.ProductSold,p.ProductCreatedDate,p.ProductStatus,p.EmployeeId,p.CategoryProductDetailsId from product as p\n"
                + "join productsize as ps on p.productid = ps.productid\n"
                + "join categorysize as cs on ps.categorysizeid = cs.categorysizeid\n"
                + "join categoryProductDetails as cpd on p.CategoryProductDetailsId = cpd.CategoryProductDetailsId\n"
                + "WHERE p.productname LIKE '%" + searched + "%' AND p.productStatus LIKE 'on'\n";
    ...
    public List<Product> filterSearchProductByProNameAndSizePaging(String price, String[] size, String searched, int indexPage) {
        List<Product> list = new ArrayList<>();
        String sql = "select Distinct p.productid, p.productname, p.ProductImageDefault, p.ProductPrice, p.ProductCost,p.ProductDescription,p.ProductRating,p.ProductSold,p.ProductCreatedDate,p.ProductStatus,p.EmployeeId,p.CategoryProductDetailsId from product as p\n"
                + "join productsize as ps on p.productid = ps.productid\n"
                + "join categorysize as cs on ps.categorysizeid = cs.categorysizeid\n"
